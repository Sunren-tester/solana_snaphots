...
service_file="/home/ubuntu/solana/solana.service"
LEDGER=$(cat $service_file | grep "\--ledger" | awk '{ print $2 }' )
SNAPSHOTS=$(cat $service_file | grep "\--snapshots" | awk '{ print $2 }' )

if [ "$LEDGER" == "" ]; then 
service_file=/home/ubuntu/solana/validator.sh
LEDGER=$(cat $service_file | grep "\--ledger" | awk '{ print $2 }' )
SNAPSHOTS=$(cat $service_file | grep "\--snapshots" | awk '{ print $2 }' )
fi

if [ "$LEDGER" == "" ]; then LEDGER=/home/ubuntu/1/ledger
fi
if [ "$SNAPSHOTS" == "" ]; then SNAPSHOTS=$LEDGER
fi
networkrpcURL=$(cat /home/ubuntu/.config/solana/cli/config.yml | grep json_rpc_url | grep -o '".*"' | tr -d '"')
if [ "$networkrpcURL" == "" ]; then networkrpcURL=$(cat /home/ubuntu/.config/solana/cli/config.yml | grep json_rpc_url | awk '{ print $2 }')
fi

catchup_info() {
  while true; do
    rpcPort=$(ps aux | grep solana-validator | grep -Po "\-\-rpc\-port\s+\K[0-9]+")
    sudo -i -u ubuntu solana catchup --our-localhost $rpcPort
    status=$?
    if [ $status -eq 0 ];then
      exit 0
    fi
...
source ./venv/bin/activate
pip3 install -r requirements.txt
python3 snapshot-finder.py --snapshot_path $SNAPSHOTS  --max_latency 100 --min_download_speed 30
systemctl start solana
catchup_info
...
echo "solana.service not found! Default: /home/ubuntu/solana/solana.service"
fi
